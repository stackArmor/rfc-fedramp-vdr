...
are a severity source registry and a small per-benchmark exception table, both
...
\item \textbf{Cloud-configuration benchmarks:} exercisable iff the finding category is
  \emph{exposure-class}, decided by the keyword rule and exception table below.
\end{itemize}

\paragraph{The keyword rule.} For cloud benchmarks the normative default reads the
category identifier itself, never the rule prose. The identifier is normalized ---
uppercased and split on non-alphanumeric runs --- and the finding is exposure-class if
any resulting token is one of $\{$\code{PUBLIC}, \code{OPEN}, \code{ANONYMOUS},
\code{INTERNET}, \code{WORLD}$\}$. Normalizing on token boundaries is what makes the
rule safe across scanner grammars: \cat{OPEN_SSH_PORT} matches on the \code{OPEN}
token, while \cat{OPENSSH_CONFIG} does not, and an AWS Config identifier such as
\code{s3-bucket-public-read-prohibited} normalizes to the same tokens as its GCP
cousin. Because the rule reads scanner-emitted identifiers, the provider cannot game
it by renaming.

The rule also matches two literal patterns before tokenizing: an identifier
containing the literal string \code{0.0.0.0} or \code{::/0} is exposure-class. The
any-address CIDR is unambiguous exposure semantics, and it is caught by literal
substring rather than by token matching precisely because normalization would split
\code{0.0.0.0} into a bare \code{0} and lose the meaning. An identifier such as
\code{sg-allow-ingress-from-0.0.0.0-0} therefore matches, while a plain numeric
segment in a category like \cat{TLS_1_0_ENABLED} does not (it carries no any-address
literal and no exposure token).

The normative spec keeps only what a second implementer needs to reproduce a result:
identifier-based matching (never prose), normalization, the token set and the
\code{0.0.0.0}/\code{::/0} literal patterns, the vocabulary fail-loud rule, and the
exception-table governance below. The reasoning behind which tokens made the set is
...
\paragraph{The exception table and its governance.} The keyword rule is refined by a
per-benchmark-version exception table with four kinds of entry: \code{add} (genuine
exposure rules the keyword rule misses), \code{remove} (keyword false positives),
\code{surface} (categories exercisable only when the attached resource is public,
joined from the same scan's exposure findings; unknown attachment resolves to public),
and \code{vocabulary} (the version's complete category list). The vocabulary closes an
otherwise silent gap: a scanned category that is \emph{present} in the vocabulary but
untagged is treated as identity/operations-plane and not exercisable --- a disclosed
calibration choice --- whereas a category \emph{absent} from the vocabulary (a
scanner's monthly new addition, say) fails loud to LEV+IRV and is flagged as
unreviewed. The governance around the table is what makes it trustworthy rather than
a hidden lever:
...
\item Removing a token-matching category is a hard assessor-review trigger --- it is
  the one move that silently slows a clock, so it never happens silently.
\item Keeping the tables current is an operating requirement with a monthly
  new-category diff, not a once-per-version event.
\end{itemize}

Categories that neither match a token nor appear in the exception table are
identity- or operations-plane and are \emph{not} exercisable. The justification is
the test's own precondition clause: a user-managed service-account key requires
possessing the secret, and the public cloud control plane alone gives an internet
actor nothing to exercise. A data-plane grant to \code{allUsers} or
\code{allAuthenticatedUsers}, by contrast, \emph{is} exposure-class regardless of the
category name --- \code{allAuthenticatedUsers} is satisfied by any self-registered
account, which the unauthenticated test treats as self-suppliable.
...
determination together with the exception-table content hash and the enumerated
...
(\cat{USER_MANAGED_SERVICE_ACCOUNT_KEY}).} No exposure token, not in the exception
table, present in the vocabulary: identity/operations-plane
...
\paragraph{Example 2 --- public bucket (\cat{PUBLIC_BUCKET_ACL}).} Exposure-class by
the \code{PUBLIC} token ${}\rightarrow{}$ \textbf{LEV+IRV}. Governed SCC severity
...
\appendix
\section{Worked exception table: CIS GCP Foundation 2.0}
\label{app:table}
The exception table below is the canonical artifact for the CIS Google Cloud Platform
Foundation~2.0 benchmark as emitted by Google Security Command Center. It is what the
back-test of Section~\ref{sec:backtest} ran against. In production the table is
content-hashed and the hash is carried in each finding's audit record; the vocabulary
is the benchmark version's complete category list, abbreviated here to the entries the
classification logic turns on.

The keyword rule alone classifies the pure-exposure categories --- their identifiers
carry an exposure token, so they need no table entry and are always exercisable:
...
\bottomrule
\end{tabular}
\caption{Keyword-matched exposure categories (no table entry required).}
\label{tab:kw}
\end{table}
...
The \code{add} and \code{remove} sets are both empty for this version: the keyword
rule covers every pure-exposure category, and after the API-key categories were moved
to \code{surface} there is no keyword false positive left to strip. Every other
