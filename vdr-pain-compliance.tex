...
column has no N3.} Table~\ref{tab:paintbl} sends Narrow to N2 in both columns and
Disruptive to N4 under multi-agency, so nothing lands on N3 when scope is
multi-agency. The line between an incident (N4 and above) and maintenance (N2 and
below) therefore sits exactly on the Disruptive boundary, and the effect matrix is
calibrated with that boundary in view.
...
already carried by \code{PUBLIC}/\code{OPEN}/\code{INTERNET} or by explicit table
entries, while their false-positive rate is high (\cat{SQL_EXTERNAL_SCRIPTS_ENABLED}
is not an internet exposure; \cat{API_KEY_APIS_UNRESTRICTED} scopes a key, not a
surface). Keeping them out holds the auto-match vocabulary tight and moves the burden
of proof onto a reviewed table entry. Screening every category list for token
collisions is a normative table-build step, not an afterthought.
