...
iff the attached resource is public; unknown attachment resolves to public), or
...
A scanned category \emph{absent} from the table's vocabulary fails loud to LEV+IRV.
...
\paragraph{Table encoding and governance.} The canonical table is encoded as four
kinds of entry, expressed as review decisions relative to the recommended generator:
\code{add} (exposure categories the generator misses), \code{remove} (generator false
positives), \code{surface} (categories exercisable only when the attached resource is
public, joined from the same scan's exposure findings; unknown attachment resolves to
public), and \code{vocabulary} (the version's complete category list). The encoding is
a convenience that concentrates review effort on deviations; what binds is the
effective per-category classification it produces. The vocabulary closes an
...
Categories present in the vocabulary but carrying no exposure or surface
classification are identity- or operations-plane and are \emph{not} exercisable. The justification is
