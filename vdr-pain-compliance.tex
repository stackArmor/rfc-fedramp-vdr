...
LEV+NIRV on an evidence-backed internal-exploitability override; NLEV otherwise. Note
what this means for the baseline: the single exercisability determination sets both
letters at once. A failed configuration needs no separate exploit-likelihood analysis
--- the misconfiguration \emph{is} the exploitable state, so exercisable means likely
exploitable \emph{and} internet-reachable for prioritization purposes, and LEV
without IRV arises only through the evidence-backed override, never from the
baseline. The
