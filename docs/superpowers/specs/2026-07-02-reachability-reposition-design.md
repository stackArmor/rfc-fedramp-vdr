...
**Steady-state consequence (added 2026-07-02):** because the evidence is
collected by the auditor in assessment and regenerated by the DAST/web-app
scanning ConMon already requires, a conforming provider has standing
sanitization evidence at all times. In that steady state the IRV set
converges to the internet-accessible set for most reporting scenarios (the
concepts stay distinct; the sets coincide in practice). The convergence is
conditional: teams must stand ready to re-prioritize and re-triage when
threat intelligence surfaces a zero-day that defeats known sanitization
techniques, WAF configurations, and other protections (Log4Shell is the
canonical case) — sanitization exclusions are suspended, findings re-enter
IRV, and the exclusion is re-earned against the new technique. FedRAMP's
broad wording describes this re-triage posture, not the everyday one.
