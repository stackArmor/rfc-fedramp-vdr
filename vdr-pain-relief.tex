...
FedRAMP does not force this: \code{FRD-LEV} defines a likely-exploitable
vulnerability as one ``not fully mitigated \emph{and} reachable by a likely threat
actor,'' and does \emph{not} make KEV-catalog membership a likely-exploitable
condition (that ``\code{KEV}$\Rightarrow$LEV'' default is ours, deliberately
conservative). Strong verified controls that lower threat-actor reachability are
therefore, in principle, consistent with an NLEV call. An adopter \rfckw{may}
enable a narrow exception: a KEV is classified NLEV only if
...
\paragraph{Impact --- a least-privilege credential bounding SQL-injection
escalation.} A \code{CWE-89} finding scores $C=I=$ High on a datastore. Read what
that High assumes: a SQL injection running with a \emph{broad} credential can
reach every schema and tenant, dump the credential tables (theft, then lateral
movement), use file/COPY functions to touch the host, or load an extension for
code execution. The application instead connects with a credential confined to its
own schema --- no superuser, no cross-database or system-catalog reach, no file or
extension privileges --- verified from a grants dump. A successful injection is
now bounded: it cannot escalate to the database instance, other tenants, stored
credentials, or the OS. \emph{This is the important limit, and it is exactly where
the objection lands:} least-privilege bounds where the injection can \emph{go}, not
the data the application is entitled to \emph{hold}. So the row lowers Modified
Confidentiality and Integrity High$\to$Low only to the extent the app's own schema
is less sensitive than that broader reach --- and the row's disqualifier fires
precisely when it is not: if the reachable schema is itself the crown-jewel data
the finding is about, scope and blast radius coincide and the row grants nothing.
In practice this ties to the asset's own criticality: a top-sensitivity datastore
(its own data \emph{is} the target) earns no confidentiality relief here; a
service whose High reflects escalation potential does. When the credit does apply,
the finding lands at N3; the reachability answer is unchanged.
...
decrypts and checks the Host header, catching domain fronting --- $0.50$.
Log4Shell is a KEV, so \emph{by default} it is frozen at LEV: the residual applies
to nothing and the CISA clock governs. \emph{With the KEV exception enabled},
though, break-and-inspect ($\rho=0.5$) drops Log4Shell's adjustedEPSS from
$\approx0.94$ to $\approx0.47$, clearing both bars ($\rho\le0.7$,
$\mathrm{adjustedEPSS}<0.5$), so it may be classified NLEV. That relaxes the
\code{VDR-TFR-PVR} cell from the tight LEV column toward the NLEV column --- but the
CISA KEV due date remains the binding floor, so the deadline moves down only
\emph{to} that date, never past it: the extra-tight cell beneath the CISA deadline
is removed, the deadline itself is not. A non-KEV fetch-dependent RCE at EPSS
$0.74$ behind the same control lands at $0.74\times0.5=0.37$ and moves to NLEV
outright --- no KEV floor, its full clock earned --- published EPSS untouched
beside the adjusted value in both cases.
