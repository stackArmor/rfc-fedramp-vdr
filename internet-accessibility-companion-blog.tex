...
\item \textbf{Whose login is in front of it?} This is where intuitions most often
  fail. A VPN or identity-aware proxy that exists to let \emph{your own people} in
  --- admin consoles, operations tooling, internal dashboards --- is a real
  boundary, and it excludes: strangers are rejected before the backend sees a
  byte. But when the login wall is the front door of the customer application,
  authentication excludes nothing: internet data still flows through the login
  and into the backend. SQL injection behind a customer sign-in page is still
  internet-reachable. The difference is the boundary's \emph{role}, not the size
  of the crowd behind it --- a change-management tool behind an IAP at a large
  provider can have a hundred thousand authorized users and still be remote
  access. And deciding which role a given login plays is a human call: the
  operator declares it, and the auditor validates it. Two more rules keep the
  exclusion honest. The boundary must be a \emph{separate component} --- an IAP
  or a VPN, never the application's own login page, because a login page is the
  application itself parsing stranger input. And the app behind the boundary
  still needs its \emph{own} login, exactly like tools behind a VPN do: if the
  app accepts the proxy's signed headers as the entire login, then one hijacked
  proxy session --- or a compromised proxy, which will happily stamp valid
  headers --- opens the app, so that app stays internet-accessible. Two doors,
  two logins. (What a customer login \emph{does} earn you is an exploitability
...
``it's behind the VPN / SSO'' & a separate edge component (not the app's own
login page) in a remote-access role --- declared by the operator, validated by
the auditor --- with the app's own independent login still behind it \\
