...
\item \textbf{Whose login is in front of it?} This is where intuitions most often
  ...
  operator declares it, and the auditor validates it. (What a customer login
  \emph{does} earn you is an exploitability argument --- it defeats FedRAMP's
  ``an automated unauthenticated system can exploit it'' floor --- which
  shortens nothing about the data path but can support a likely-exploitable
  downgrade.)
...
``it's behind the VPN / SSO'' & the boundary's role is remote access for your own
people, not the customer product's front door --- declared by the operator,
