...
chained-access case: the CAPEC relationships already linked through the
findings' CWEs can identify a candidate upstream attack pattern that explicitly
precedes a candidate downstream pattern. When the upstream finding is directly
reachable, both findings occupy the same canonical execution boundary, and a
review confirms that the downstream pattern's prerequisites are compatible,
the downstream finding may be treated as IRV for remediation timelines. The
downstream CVSS vector does not change; the finding-level IRV determination
records the upstream CVE, both CWE--CAPEC mappings, the CAPEC relationship, and
the applicability decision. A coarse asset assertion
...
findings reached through governed CAPEC-supported chains. A separate
...
continuous monitoring. It also promotes a downstream finding when an active,
directly reachable upstream finding in the same canonical execution boundary
maps through CWE to a CAPEC pattern that explicitly precedes the downstream
finding's CAPEC pattern, and deployment review confirms the relationship is
applicable. In set terms,

\[
R_{\mathrm{operational}}=D\cup(K\setminus\mathcal P_0)\cup G_C,
\]
...
prevention assertion. $G_C$ is the set of active findings promoted through an
applicable CAPEC chaining relationship from an active, directly reachable
upstream finding in the same canonical execution boundary. The
...
control is currently supported. The missing cases and chain-candidate review
remain disclosed limitations. CAPEC provides a stronger signal than an
\code{AV:N}-plus-CWE rule, but it still describes attack-pattern relationships
rather than a deployment-specific exploit proof; better upstream metadata is
the durable fix.
\end{plain}

\section{Current Inputs and Optional Refinement}
\label{sec:gaps}
The recommended method joins vulnerability trigger and CAPEC chain metadata
with deployment evidence at the resolution available today. Trigger and CAPEC
mapping are chiefly vulnerability-level facts; exposure, co-residency,
execution boundary, chain applicability, and prevention are provider-level facts.
...
\subsection{CAPEC-supported chain metadata}
CAPEC supplies machine-readable \code{CanPrecede} and \code{CanFollow}
relationships between attack patterns and machine-readable \code{Related
Weaknesses} links to CWE. The two relationship directions are normalized into
one directed edge: if pattern $q$ \code{CanFollow} pattern $p$, then $p$ is
treated as \code{CanPrecede} $q$. Only active Standard or Detailed patterns are
eligible; Meta patterns and entries marked Deprecated or Obsolete are excluded
from automatic use. The corpus version is retained with every result.
\cite{capec-schema,capec-catalog}

A CVE's CWE therefore maps it to a set of eligible CAPEC patterns. A pair of
findings becomes a chain candidate only when a CAPEC pattern for the upstream
CVE has an explicit directed edge to a CAPEC pattern for the downstream CVE.
CAPEC prerequisites and execution-flow text help a reviewer decide whether the
candidate applies to the deployed products, privileges, and execution context.
They are not a formal machine-checkable \emph{provides/requires} contract, and
the CWE--CAPEC association is many-to-many and unweighted. CAPEC ATT\&CK
taxonomy mappings may label the likely adversary stage, but a shared tactic is
context rather than proof that two findings compose. This is a governed triage
relationship, not a change to either CVSS vector or proof that every mapped
pair forms an exploitable chain.
...
\begin{keybox}
\centering
\textbf{Trigger and CAPEC Chain Mapping}
\quad$+$\quad
...
\item $W(v)$ is the set of specific CWE identifiers assigned to CVE $v$, and
  $A(v)$ is the set of eligible CAPEC patterns reached from those CWEs through
  CAPEC \code{Related Weaknesses} mappings.
\item $p\rightarrow_C q$ means that eligible CAPEC pattern $p$ explicitly
  \code{CanPrecede} eligible pattern $q$, after normalizing an inverse
  \code{CanFollow} statement. $H_C(u,v)=1$ when such a pair exists for
  $p\in A(u)$ and $q\in A(v)$.
...
\item $L_0(u,v,x,a)$ is the governed applicability decision for the proposed
  chain. It is 1 only when the findings are active in the same boundary and a
  review confirms that the source outcome and target prerequisites are not
  contradicted by the deployed products, privileges, architecture, sandbox, or
  execution context. It retains evidence and reviewer provenance.
\end{itemize}

$G_C(v,a)$ is the binary, CAPEC-supported chain-promotion indicator for downstream
...
D(u,x)=1\land H_C(u,v)=1\land L_0(u,v,x,a)=1
\right].
\label{eq:chain}
\end{equation}
...
In plain terms: set $G_C(v,a)=1$ when at least one active, directly
internet-reachable finding in the same execution boundary maps to a CAPEC
pattern that explicitly precedes a CAPEC pattern mapped to the downstream CVE,
and the deployment review confirms that this particular transition applies.
Otherwise set $G_C(v,a)=0$.
\end{plain}

An active upstream finding must itself match a public network surface, not
merely carry an \code{AV:N} vector. The downstream finding may be
\code{AV:L}, \code{AV:A}, \code{AV:P}, or even a separately exposed
\code{AV:N} finding; $G_C$ evaluates the chain independently and never rewrites
that vector. The record preserves the source and target CVEs and CWEs, source
and target CAPEC IDs, normalized relationship, canonical boundary, CAPEC corpus
version, applicability evidence, reviewer, and policy version. When the last
qualifying upstream finding is resolved, loses direct exposure, leaves the
execution boundary, or no longer satisfies the reviewed prerequisites,
$G_C$ is recomputed and only the chain-derived promotion is withdrawn.
...
\paragraph{CAPEC chain classification.}
The enrichment table is built offline from the CAPEC XML rather than by live
per-finding traversal. For each CWE, it records the eligible CAPEC IDs and all
normalized outgoing and incoming chain relationships. At ingest, each CVE's
specific CWE assignments are joined to that table. Candidate finding pairs are
generated only within the same canonical execution boundary and only when the
upstream finding satisfies $D(u,x)=1$. The pair is eligible for a positive
$L_0$ review only when an explicit CAPEC edge exists; sharing a CAPEC pattern,
an ATT\&CK tactic, or a broad weakness family is insufficient.

The CAPEC edge answers ``does MITRE describe this attack-pattern ordering?''
It does not answer ``does exploiting this exact source CVE produce everything
this exact target CVE requires on this deployment?'' CAPEC prerequisites,
execution-flow steps, consequences, vendor guidance, exploit analysis, and the
deployed execution context supply that second decision. Many-to-many or
ambiguous mappings remain in \emph{Chain review} until a reviewer selects a
specific defensible path. Only a positive, provenance-backed $L_0$ decision
sets $G_C=1$ and therefore uses the IRV remediation column for the downstream
finding.

\begin{keybox}
\textbf{Operational chain mapping.}
\[
\mathrm{CVE}_u\rightarrow\mathrm{CWE}_u\rightarrow\mathrm{CAPEC}_p
\xrightarrow{\mathrm{CanPrecede}}
\mathrm{CAPEC}_q\leftarrow\mathrm{CWE}_v\leftarrow\mathrm{CVE}_v
\]
The CAPEC XML supplies both CWE joins and the directed pattern edge. The
provider then adds the facts the catalog cannot know:
\[
D(u,x)=1\quad\land\quad B(x)=B(a)\quad\land\quad
\mathrm{prerequisites\ reviewed}\quad\Longrightarrow\quad G_C(v,a)=1.
\]
The result means ``chain-derived IRV for remediation timing,'' not ``the
downstream CVE has become \code{AV:N}.''
\end{keybox}

For an easier first implementation, \code{AV:N} plus co-residency, high impact,
or a selected CWE may seed the review queue. Such a rule has materially more
false positives because it does not identify a target pattern or an ordering
relationship. It must never set $G_C=1$ by itself. \code{AV:N} remains useful
in its proper role as evidence for the direct-network test $D$.
...
to exploit $v$. This separates a demonstrated capability match from the
operational $G_C$ decision, which uses CAPEC as a candidate transition and a
reviewed $L_0$ decision as the deployment-specific compatibility gate.
...
prevention assertion $P_0(v,a)$, plus the CAPEC candidate $H_C(u,v)$,
applicability decision $L_0(u,v,x,a)$, and boundary-scoped chain branch
$G_C(v,a)$. The full-information form shows
...
\item[Confirmed IRV] A direct match exists, or a promoted indirect trigger
  ...
  promoted by an applicable CAPEC-supported chain from a directly reachable
  finding in the same execution boundary.
\item[Provisional IRV] An applicable trigger is known, but deployment or
  prevention evidence is incomplete.
\item[Chain review] A CAPEC edge exists, or a lower-confidence fallback rule
  suggests a pair, but the exact mapping, prerequisites, execution context, or
  boundary evidence is incomplete. It does not promote the downstream finding.
...
a downstream finding only when a directly reachable source finding in the same
canonical execution boundary has an explicit, eligible CAPEC transition to the
target finding and the deployment-specific applicability review is positive.
This can temporarily miss an undocumented CAPEC edge or chain and can still
over-promote when the pattern-level relationship is broader than the two CVEs'
actual exploit semantics. Threat-intelligence
...
co-residency still does not set $G_C=1$ when no explicit, applicable CAPEC
transition exists.

\item \textbf{CAPEC-supported chain inside one execution boundary.}
A public service has active source CVE $u$ and $D(u,x)=1$. Its assigned CWE
maps to eligible CAPEC pattern $p$. A second active finding $v$ in the same
canonical boundary maps through its CWE to eligible CAPEC pattern $q$, and the
catalog explicitly states $p\rightarrow_C q$. The pair enters \emph{Chain
review}. Vendor and exploit evidence confirm that exploiting $u$ establishes
the privilege and execution context listed in $q$'s prerequisites, so the
reviewer records $L_0(u,v,x,a)=1$. Equation~\ref{eq:chain} then sets
$G_C(v,a)=1$, making $v$ IRV for remediation timelines without changing its
original CVSS vector. The record identifies both CVEs, both CWE--CAPEC paths,
the directed CAPEC edge, the boundary, and the review evidence.

If the patterns merely share an ATT\&CK tactic, no explicit CAPEC edge exists,
the prerequisites conflict, the source is not actually exposed, or the
findings occupy different boundaries, the pair does not promote. An
\code{AV:N}-plus-CWE fallback may still surface it for review, but cannot set
$G_C=1$. Removal of the last qualifying source finding triggers recomputation.
...
the assessment and ConMon program. Its third is CAPEC coverage and abstraction.
CVE-to-CWE and CWE-to-CAPEC mappings are incomplete, many-to-many, and
unweighted; explicit chain edges are sparse and pattern-level prerequisites are
prose. Filtering out Meta and retired patterns improves precision but reduces
coverage. Even an explicit CAPEC edge does not prove that one specific CVE
supplies another CVE's exact local prerequisites, which is why $L_0$ is a
required gate. Mapping gaps can miss real chains; broad mappings or weak review
can over-classify them. Stronger exploit-outcome and local-precondition metadata
would permit the full-information $Q\cap X_Q$ comparison.
...
\item coverage and ambiguity of CVE--CWE--CAPEC joins after abstraction and
  status filtering;
\item precision and recall of explicit CAPEC chain candidates $H_C$;
\item false-positive and false-negative rates for boundary-scoped $G_C$
  promotion, broken out by CAPEC edge, prerequisite review, execution context,
  and optional \code{AV:N}-plus-CWE review-queue seeds;
...
indirect internet-content trigger or exactly which exploit outcomes satisfy a
second CVE's prerequisites. CAPEC supplies a useful, existing bridge from CVE
through CWE to directed attack-pattern relationships, but does not resolve that
deployment-specific compatibility by itself. More detailed deployment evidence can refine
...
direct branch and uses CVSS attack vector for direct matching without rewriting
the downstream vector, promotes known indirect
triggers once per CVE, and joins them to coarse content-receipt classes. It
separately promotes a finding when an explicit CAPEC transition connects it to
a directly reachable source finding in the same canonical execution boundary
and a deployment review confirms compatible prerequisites, while retaining the
downstream vector and full chain provenance. It
...
Three explicit rules make the method scalable. Unknown indirect triggers
...
evidence or change revokes it; and $G_C$ treats an explicit CAPEC edge as a
candidate that promotes only after a positive applicability decision. These
create residual risk from missing trigger metadata, incomplete or stale control
evidence, sparse or broad CAPEC mappings, and false-positive or false-negative
applicability and boundary classification. Threat intelligence, KEV handling,
...
provenance-backed trigger and exploit-outcome profiles once and carry them
through scanners so providers can promote exceptional indirect cases and
validate CAPEC chain candidates consistently. Agentic
...
\bibitem{capec-schema}
MITRE, \emph{CAPEC Schema Documentation}, related weaknesses, related attack
patterns, chaining relationships, prerequisites, execution flow, and taxonomy
mappings.\\
\href{https://capec.mitre.org/documents/schema/schema_v3.0.html}{CAPEC schema documentation}

\bibitem{capec-catalog}
MITRE, \emph{Common Attack Pattern Enumeration and Classification}, Version
3.9 catalog and machine-readable downloads.\\
\href{https://capec.mitre.org/data/index.html}{CAPEC 3.9 catalog}
...
CAPEC & Common Attack Pattern Enumeration and Classification & ATT\&CK & Adversarial Tactics, Techniques, and Common Knowledge \\
