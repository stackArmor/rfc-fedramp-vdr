...
governed, versioned high-recall classifier over CVSS attack vector,
vulnerable-system impact, and a narrow CWE seed set. The record must retain the
status (high confidence, possible, or none), exact reason codes, source facts,
policy version, and any analyst override. It is a triage classification, not a
change to the CVSS vector or an assertion that every matching CVE is proven
...
automatically assigns high confidence when either (1) CVSS v3 reports
\code{C:H/I:H/A:H}, or CVSS v4 reports the vulnerable-system equivalent
\code{VC:H/VI:H/VA:H}; or (2) the CVE maps to one of the strict execution
semantics seeds \code{CWE-78}, \code{CWE-94}, \code{CWE-95}, \code{CWE-96},
\code{CWE-98}, \code{CWE-553}, \code{CWE-624}, or \code{CWE-917}. The reason
record distinguishes the full-impact rule from the matched CWE.\cite{cwe-catalog}

The full-impact rule is a deliberate high-recall presumption, not proof of
remote code execution; full confidentiality, integrity, and availability
impact can arise by another mechanism. A second, non-promoting \emph{possible}
set routes \code{AV:N} findings mapped to \code{CWE-97}, \code{CWE-1336},
\code{CWE-494}, \code{CWE-829}, or \code{CWE-470} for review because execution
depends on server-side use or other CVE-specific conditions. Broad command
languages, argument injection, dangerous upload, deserialization, exposed
dangerous functions, and memory-corruption families likewise require explicit
CVE-level execution evidence; their CWE alone does not establish a chainable
entry point. MITRE itself cautions that code execution is a consequence of many
root-cause weaknesses and is not, by itself, a reason to map a CVE to
\code{CWE-94}.\cite{cwe-catalog}

Both seed sets are governed routing policy, not semantic definitions of remote
code execution. They must be versioned, auditable, and replaceable by stronger
CNA, vendor, Vulnrichment, exploit, or approved analyst evidence. A possible
entry point remains visible for filtering and review but does not set $G_0=1$.
...
Its vector is \code{AV:N/C:H/I:H/A:H}, so the governed full-impact rule assigns
$E_0(u)=\mathrm{high}$ and retains that exact reason. The same host/kernel
boundary has an active \code{AV:L} kernel finding. Equation~\ref{eq:chain}
sets $G_0(v,a)=1$, making the kernel finding IRV without changing its
\code{AV:L} vector. The record identifies the upstream CVE that caused the
promotion. A possible-only entry point, an unexposed \code{AV:N} package, a
resolved upstream finding, or a finding in a different runtime boundary does
not produce the automatic promotion. Removal of the last qualifying upstream
finding triggers recomputation.
...
the assessment and ConMon program. Its third is the deliberately high-recall
chain classifier: neither full vulnerable-system H/H/H impact nor a CWE seed
proves that an upstream CVE supplies compatible local execution, and $G_0$
presumes compatibility for active \code{AV:L} findings inside the verified
boundary. This can over-classify until stronger exploit-outcome and local
precondition metadata populate the full-information $Q\cap X_Q$ comparison.
...
\item false-positive and false-negative rates for boundary-scoped $G_0$
  promotion, broken out by full-impact, CWE, and evidence-confirmed reasons;
