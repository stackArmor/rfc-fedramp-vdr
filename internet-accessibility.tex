...
\begin{plain}
Two kinds of bugs. A content-triggered bug is like a letter bomb: the danger is in the letter itself, so it goes off wherever the mail is forwarded, no matter how many desks it crosses --- a poisoned search string, a booby-trapped file. A peer-bound bug is like picking a lock: you have to be standing at the door yourself, and you cannot pick a lock by mail. Interior machines only accept connections from their own neighbors, so an internet attacker never stands at their door --- which is why handshake, login-check, and connection-flood flaws can be crossed off on machines the internet cannot connect to. Sorting a bug into the wrong pile (``XSS is client-side,'' ``DoS is peer-bound'') is exactly the reckless shortcut the rules warn against, so anything unclassifiable is treated as content-triggered.
\end{plain}
