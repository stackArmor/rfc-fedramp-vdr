...
\item \textbf{Non-Kubernetes estates:} VPC flow logs joined with host-level socket telemetry---which process held the socket, which peers it exchanged data with---giving per-process flow edges rather than bare IP adjacency.
\end{itemize}
...
Every vulnerability needs a specific trigger surface---a protocol, file format, or API endpoint whose processing sets it off. And Phase B's edges carry more than ``connected'': the flow telemetry records what is actually delivered on each edge (protocol, port, and often the application protocol, from mesh or eBPF-level classification). Filter 2 checks the two against each other:
\begin{quote}
...
\end{quote}
For example: an SMB-stack CVE on a host whose only inbound tainted edge carries PostgreSQL wire protocol is excludable---no internet-derived data ever arrives in a form the SMB code will parse. The CVE-to-surface mapping comes from enrichment data (affected component, CPE, vendor advisory). Where the required surface cannot be determined, \textbf{the filter assumes any surface could trigger the flaw, and the vulnerability stays IRV}. That default is the safety property again: incomplete knowledge widens the IRV set instead of narrowing it.
...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Payload-carried} classes are triggered by the \emph{content} of the data, and content survives any number of forwarding hops: injection (CWE-89 SQL, CWE-78 OS command, CWE-917 expression language---the Log4Shell mechanism), deserialization of untrusted data (CWE-502), memory corruption in format parsers (CWE-119/CWE-787 where the component parses delivered data), XXE (CWE-611), SSRF (CWE-918), and path traversal (CWE-22). These are exactly FedRAMP's SQL-injection and Log4Shell concern: \textbf{payload-carried vulnerabilities stay IRV wherever tainted data flows, and can never be excluded by class.} Filters 1 and 2 may still exclude them.
\item \textbf{Peer-bound} classes require the attacker to be the \emph{direct network peer} of the vulnerable listener: authentication bypass at the socket (CWE-287/CWE-306 on the listening service itself), TLS and protocol handshake or state-machine flaws, and connection-flood denial of service. A forwarded application payload cannot perform a handshake or open a raw connection to an interior listener. So \textbf{peer-bound vulnerabilities are excludable on any asset that is not directly exposed} ($\lnot E(a)$): the only machines that can connect to the listener are the interior systems Phase B identified, and none of them is an internet source.
\end{itemize}
Two cautions keep this filter honest. First, stored XSS is payload-carried: the script arrives from the internet and is faithfully forwarded through the stack, even though it finally fires in a browser. ``XSS is client-side, therefore excluded'' is wrong. Second, not all denial of service is peer-bound: ReDoS and decompression bombs are triggered by payload content and follow the data wherever it flows. A blanket exclusion of ``XSS and DoS'' is exactly the reckless evaluation \code{VER-EVA-ELX} warns against. The sound unit of exclusion is the combination in Equation~\ref{eq:decision}---class, position, and taint together---and any vulnerability whose CWE is missing or fits neither group defaults to payload-carried and stays IRV.
...
\label{sec:waf}
The last design question: how should a provider represent vulnerabilities shielded by a Web Application Firewall or a comparable payload-inspecting perimeter (virtual patches, OWASP core rule set deployments)? Two approaches compete:
\begin{enumerate}[leftmargin=1.5em, nosep]
...
The FedRAMP VDR/VER rules ask a sharper question than the industry is used to answering: not ``which resources are internet-accessible?'' but ``which \emph{vulnerabilities} can a payload from the public internet trigger?'' (\code{FRD-IRV}, \code{VER-EVA-EIR}). The Exclusion-Gated Reachability Method answers it in three deterministic phases. Phase A's gates---routing, attributed firewall sources, remote-access boundaries, container routing, and the process-level refinement---find where internet data enters. Phase B follows that data across the observed flow graph and prunes, wholesale and with flow logs as evidence, the assets it can never reach. Phase C excludes individual vulnerabilities on the remaining assets through execution evidence, trigger-surface matching, and the payload-carried/peer-bound split. Every filter defaults toward IRV, so evidence gaps show up as conservatism, never as silent false negatives---and every exclusion carries machine-verifiable evidence and a VEX justification a 3PAO or agency can check.
