...
\paragraph{Where the other mitigations land.} The WAF earns its own section only because it is the one control FedRAMP lets a provider count against \emph{reachability}. Every other standard mitigation moves a different lever, and the sorting test is always the same question: \emph{what does the control actually stop?}
\begin{itemize}[leftmargin=1.5em, nosep]
\item \textbf{It stops the payload from arriving.} Network segmentation that removes the data path, or a qualifying remote-access boundary (Gate~3). These are reachability facts: Phase~B re-derives the finding NIRV from topology evidence, and no VEX claim is needed.
\item \textbf{It stops the flaw from firing at all.} A configuration that disables the vulnerable feature (Log4Shell's \code{formatMsgNoLookups} flag), a removed package, a function that never executes (Filter~1). These are \emph{disposition} facts: the finding is attested \code{not\_affected} with the matching justification (\code{requires\_configuration}, \code{code\_not\_present}, \code{code\_not\_reachable}) and leaves the active queue with its PAIN retained, per the companion method memo. Reachability does not change---the payload still arrives; it just has nothing to set off.
\item \textbf{It blocks the exploitation technique at runtime.} Enforcing runtime controls---RASP, or a seccomp or AppArmor profile that denies the system calls the exploit needs---are the WAF's siblings: same posture (keep the finding IRV, attest \code{protected\_at\_runtime}) and same fragility rules, because a bypassed or disabled profile re-exposes the flaw with no signal in the vulnerability record. Detection-class EDR belongs in none of these buckets: noticing exploitation is response evidence, not prevention, and it justifies no suppression at all.
\item \textbf{It shrinks the damage when the flaw fires.} Non-root users, read-only filesystems, egress denial, scoped credentials, compiler and OS hardening. These move the \emph{impact} lever, not reachability: the companion method memo's mitigation ladder lowers the Modified C/I/A metrics on cited evidence (\code{protected\_by\_mitigating\_control}, \code{protected\_by\_compiler}), and the finding descends to a lower PAIN rating on a longer clock.
\end{itemize}
Every one of these claims is control-based, so the WAF discipline applies across the board: attest it in signed VEX, cite the evidence, and re-evaluate the moment the control changes.
