...
\item \textbf{Enforcing runtime protection} (RASP, or a seccomp or AppArmor profile that denies the system calls the exploit needs): the WAF's sibling, but not an interception---the dividing line is where the control acts, and a runtime control fires \emph{while} exploitation is already underway, not before the payload is processed. Same posture---keep the finding IRV, attest \code{protected\_at\_runtime}---and same fragility rules, because a bypassed or disabled profile re-exposes the flaw with no signal in the vulnerability record. Detection-class EDR is weaker still: noticing exploitation is response evidence, not prevention, and it justifies no suppression at all.
