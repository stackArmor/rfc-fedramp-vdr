...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Content-triggered} classes are set off by the \emph{content} of the data, and content survives any number of forwarding hops: injection (CWE-89 SQL injection, CWE-78 OS command injection, CWE-917 expression-language injection---the Log4Shell mechanism), deserialization of untrusted data (CWE-502), memory corruption in format parsers (CWE-119 buffer overflow and CWE-787 out-of-bounds write, where the component parses delivered data), XML external entity injection (CWE-611), server-side request forgery (CWE-918), and path traversal (CWE-22). These are exactly FedRAMP's SQL-injection and Log4Shell concern: \textbf{content-triggered vulnerabilities stay IRV wherever tainted data flows, and can never be excluded by class.} Filters 1 and 2 may still exclude them.
\item \textbf{Peer-bound} classes work the other way around: the flaw lives in the \emph{connection}, not in the data. A network connection is a conversation between exactly two machines, and some flaws can only be triggered by the machine actually holding that conversation---the direct network \emph{peer}---because they sit in the mechanics of the conversation itself: the TLS or protocol handshake that opens it, the service's check of who it is talking to (CWE-287 improper authentication and CWE-306 missing authentication for a critical function, on the listening service itself), or the sheer number of conversations the service can hold at once (connection-flood denial of service). Data forwarded from the internet cannot trigger these, because forwarding passes along the \emph{contents} of a conversation, never the conversation itself; an interior listener only ever converses with the interior systems next to it. So \textbf{peer-bound vulnerabilities are excludable on any asset that is not directly exposed} ($\lnot E(a)$): every machine that can open a connection to the listener is one of the interior systems Phase B identified, and none of them is an internet source.
\end{itemize}
