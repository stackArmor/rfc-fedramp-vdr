...
The architectural answer is \textbf{asset archetypes.} Instead of hand-rating every server in
isolation, you sort each asset into a short catalog of roles based on \textbf{what
it does inside the service} --- a database, a login service, a cache, a
message bus, a CI/CD runner, a public load balancer, and so on. Each archetype
comes with an uncapped CR/IR/AR profile \emph{pre-filled and justified}. Tag an
asset with its archetype, and its architectural profile is inherited
automatically.
...
consequences are in scope $\rightarrow$ \textbf{classify the asset (archetype)}
--- what it does inside the service $\rightarrow$ \textbf{take the lower value
...
\item \textbf{It's an example, not a standard.} The proposal publishes a sample
  ...
  from its own architecture, roles, trust relationships, and control functions.
  Agency FIPS-199 and information-type context belongs in the separate ceiling.
  Two providers using the same software for different roles can legitimately
  assign different archetypes; two agencies can retain the same archetype while
  receiving different effective requirements.
...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{On a crown-jewel datastore whose archetype and intended-use ceiling
  retain the necessary High requirements, holding multiple agencies' data:} scores
...
\item \textbf{With governed inputs} --- an intended-use ceiling, the asset's
  archetype, and a one-agency-or-many affected scope --- every finding gets a
