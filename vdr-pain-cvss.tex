...
the directly computable network branch from the transitive content branch. Its
current operational profile is:
\[
\mathrm{IRV}_0(v,a)=
...
$P_0$ to zero. CVSS \code{AV:L/A/P} excludes a
finding from the direct branch but does not prevent promotion into the indirect
branch. The paper recommends this operational profile under today's metadata
conditions, under which reachability converges operationally toward direct
accessibility plus known uncovered indirect triggers. A separate, non-normative
...
\item \textbf{Impact-only severity, normalized by $0.915$.} We take CVSS's
  ...
  choice and every downstream CVSS component the method omits.
