...
environment. PAIN's affected-agency scope asks a different question: whether the
finding's evidenced effect reaches one definite agency or several. The
multi-agency flag in Eq.~\eqref{eq:pain} carries that concern directly.

CVSS Scope is retained as evidence for cross-component target review and for
scoping remediation or mitigation; it is neither a PAIN severity multiplier nor a
substitute for affected-agency scope. Because the concepts are not semantically
interchangeable, PAIN does not incorporate CVSS Scope's Environmental-score branch
or rename it as a multi-agency field.
...
\begin{reality}
  ...
  distinct from PAIN's \emph{multi-agency} blast-radius determination, but it can
  support an affected-agency value of several when the evidenced consequence lands
  in a security boundary or asset whose compromise can affect multiple agencies.
\end{reality}
