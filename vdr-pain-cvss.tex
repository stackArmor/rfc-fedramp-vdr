...
calculations. We recommend role-based ``asset archetypes'' as the preferred way to
assign CR/IR/AR systematically. An archetype records what a system does and derives
its requirements through a governed mapping; that is easier to verify than asking
an owner to declare the system High, Medium, or Low value. Each Cloud Service
Provider (CSP) \rfckw{should} derive its own archetype mapping from its system
...
The numerical weights and cap in Eq.~\eqref{eq:isc} are taken from the CVSS v3.1
specification. The normalization, word thresholds, and multi-agency mapping are
this method's additions; Section~\ref{sec:cvss-divergence} delineates that boundary
precisely.
...
\item $\mathrm{CR},\mathrm{IR},\mathrm{AR}$ --- how much \emph{this system} needs
  secrecy, correctness, and uptime (from its archetype, asset value, or other governed
  asset metadata). Low / Medium / High become $0.5$ / $1.0$ / $1.5$.
...
\begin{plain}
\textbf{Reading Eqs.~\eqref{eq:isc}--\eqref{eq:s} step by step.}
...
The $\min(\ldots,0.915)$ cap is CVSS's, not ours. The largest unweighted impact
combination is all-High:
$1-(1-0.56)^3=0.915$. High requirements multiply each dimension by $1.5$, so an
individual product can reach $0.84$, and the three-dimension combination reaches
approximately $0.996$. The cap prevents that adjusted result from exceeding
CVSS's global maximum MISS. The CVE determines which dimensions are affected and
their baseline magnitude; the asset requirements determine how strongly those
dimensions count. Asset criticality can move a finding toward the scale's worst
case, but not beyond its fixed ceiling. That fixed, specification-defined maximum
also gives Eq.~\eqref{eq:s} a stable denominator: $S=1.0$ means ``maximum
attainable MISS,'' and the word thresholds retain the same meaning across assets
...
Equation~\eqref{eq:isc} is algebraically identical to the CVSS v3.1 Modified
Impact Sub-Score (MISS), with $C/I/A$ denoting the effective Modified impact
values $\mathrm{MC}/\mathrm{MI}/\mathrm{MA}$ --- or their inherited Base values
when the Modified metrics are Not Defined. The metric weights, requirement
multipliers, complement-product formula, and $0.915$ cap are standard CVSS; the
abbreviated notation is ours. The departures begin after MISS, with what this
method does --- and deliberately does not do --- with that intermediate.

In the v3.1 specification, MISS feeds the full Environmental Score. For Modified
...
Environmental Score. We stop at MISS and do not carry any of that downstream
machinery into the severity term. Three decisions follow, each with a reason.
...
\paragraph{We normalize instead of computing the 0--10 blend.}
CVSS transforms MISS, together with its other inputs, into a standardized 0--10
Environmental Score. Our deliverable is a classification --- four words and five
N-levels --- so the scalar exists only to be bucketed. Equation~\eqref{eq:s}
rescales MISS to the natural axis for drawing those lines: $S$ is the fraction of
the maximum attainable MISS. A threshold of $\theta_3=0.80$ therefore means ``the
cut point is 80\% of maximum MISS.'' It is not a claim that the vulnerability
causes 80\% of some physically measurable damage; the CVSS coefficients are not a
ratio-scale measurement of harm. The division changes no ordering --- it relabels
the axis. The substantive choice is stopping at MISS.
...
whether the affected asset carries one agency's data or several agencies' data.
The multi-agency flag in Eq.~\eqref{eq:pain} carries that concern directly. Because
the two concepts are not semantically interchangeable, the CVSS Scope machinery is
omitted rather than renamed or repurposed.

The result is not a conformant CVSS Environmental Score. It reuses the standard
MISS intermediate, routes exploitability and reachability to FedRAMP's remediation
columns, represents multi-agency blast radius explicitly, and normalizes only for
classification. A reviewer verifying the CVSS provenance can compare
Eq.~\eqref{eq:isc} with the published MISS formula; every choice after it is
enumerated above and in Appendix~\ref{app:opinions}.
...
&\quad\lor\ \big(\text{IRV}_{\text{direct}} \land \text{\code{AV:N}} \land \text{\code{AC:L}} \land \text{\code{PR:N}} \land \text{\code{UI:N}}\big),\\
\text{IRV} \;&=\; \text{the \emph{finding} is internet-reachable (evaluated per vulnerability, not per asset).}
\end{align*}
The third LEV disjunct implements the \code{FRD-LEV} floor note that ``any
vulnerability that an automated unauthenticated system can exploit over the internet
is a likely exploitable vulnerability.'' The deterministic operational profile maps
that floor to a finding that is IRV via \emph{direct} exposure and whose CVSS vector
permits low-complexity, unauthenticated automation
(\code{AV:N/AC:L/PR:N/UI:N}). A finding outside this vector floor remains LEV when
either of the independent EPSS or KEV predicates is satisfied.
...
Eq.~\eqref{eq:isc}, with the detected facts as the recorded evidence. The one
...
qualifying remote-access boundary (the companion model's Gate~3) defeats the
\code{FRD-LEV} unauthenticated-automation floor. \code{VDR-TFR-PVR} is satisfied by landing on
\emph{any} longer clock, whether the step down was a row or a column. The textual
hook for the column half is \code{FRD-PMV}/\code{FRD-FMV}: FedRAMP defines
mitigation over \emph{likelihood or} PAIN, and an evidence-backed reachability or
exploitability reduction is the likelihood half.
...
at N5 also remediating in \textbf{12 hours}: a content-triggered DoS in
...
\begin{plain}
Rather than ask an owner to rate every server High, Medium, or Low, we sort systems
into a short list of \emph{archetypes} --- a database, a login service, a cache, a
message bus, and so on. Each archetype comes with its security requirements
pre-filled. Tag a system with its archetype and the scoring is automatic. Our list
is only an \emph{example}: every provider should tailor its own to the roles its
systems perform and the data they actually hold.
\end{plain}
The model in Section~\ref{sec:math} is only as good as the CR/IR/AR assignment that
feeds it. A direct value label asks the asset owner to state the conclusion: how
important is this system? That answer can be difficult to obtain and difficult to
challenge, especially when a High designation brings more work, cost, or scrutiny.
An archetype asks narrower questions about facts that architecture and inventory
can help verify: does the asset control other systems, hold credentials,
authenticate users, store sensitive data, or support several services?

The governed archetype mapping then turns those facts into CR/IR/AR. A reviewer can
check both parts of the decision: whether the asset performs the stated role and
whether that role maps to the right requirements. Archetypes do not remove human
judgment, but they make the judgment visible and harder to manipulate. Tagging an
asset with its archetype also avoids evaluating every asset in isolation and keeps
the rationale for its CR/IR/AR readily understandable and auditable. Not every
asset will fit cleanly, so CSPs \rfckw{may} define custom archetypes or adjust the
scoring of existing ones to map their estate accurately.

A simpler but less informative conforming implementation is to tag assets directly
with an \code{asset-value} instead of an \code{asset-archetype}. Under that approach,
...
\label{sec:seccon}
The method trusts the archetype tags on each system: if someone mislabels a critical
...
\rfckw{should} be controlled like any other security setting. Archetypes narrow but
do not eliminate this risk: a claimed role can be compared with architecture, data
flows, identity permissions, and inventory, while a direct value label provides
less evidence to check. Finally, the method does not detect vulnerabilities; it
prioritizes the output of a scanner and inherits that scanner's coverage and accuracy.
...
asset its CR/IR/AR and evaluating the two VER axes, a provider obtains a
deterministic, auditable, and standards-grounded PAIN and remediation deadline for
every finding. We recommend role-based archetypes as the starting point because
they record what an asset does and expose how its requirements were derived. A
direct asset-value label remains a conforming fallback, but it records the
conclusion with less evidence behind it. We offer the example archetype catalog not
as a standard but as an existence proof; we encourage each CSP to publish its own.
...
degrading. The same availability-High CVE therefore scores Debilitating on the
edge and Disruptive on the application tier, which matches how an operations team
actually experiences the two events. Third, the edge itself is split by what it
...
\item \textbf{PAIN word thresholds $(\theta_1,\theta_2,\theta_3) = (0.25,\,0.55,\,0.80)$.}
  ...
  the result as governed configuration. The companion paper
  \href{https://stackarmor.github.io/rfc-fedramp-vdr/vdr-pain-calibration.pdf}
  {\emph{Calibrating PAIN Without Abandoning CVSS}} provides a more evidence-backed
  calibration: it derives the boundaries from stated consequence scenarios,
  exhaustively enumerates all 729 CVSS impact/requirement states, and validates the
  result against anonymized operational datasets. Its revised governed calibration
  set is $(0.2811515969,\,0.5623031939,\,0.933)$ with a High-centered PAIN
  normalization denominator of $0.995904$; the values above remain the initial
  publication baseline for historical reproducibility.
...
\item \textbf{LEV is a union, not a blend.} A finding is Likely Exploitable if
  ...
  low-complexity, unauthenticated automation (\code{AV:N/AC:L/PR:N/UI:N}) ---
  whichever fires, with no weighting among them. The third disjunct is the method's
  deterministic operational profile for the \code{FRD-LEV} floor note that ``any
  vulnerability that an automated unauthenticated system can exploit over the
  internet is a likely exploitable vulnerability.'' An \code{AC:H} finding does not
  enter LEV through this vector floor alone, but KEV membership or EPSS
  $\ge \theta_{\text{epss}}$ independently places it in LEV.
\item \textbf{Impact-only severity, normalized by $0.915$.} We take CVSS's
  \emph{Modified Impact Sub-Score} and rescale it to $[0,1]$ rather than computing
  the full Environmental Score; Section~\ref{sec:cvss-divergence} enumerates this
  choice and every downstream CVSS component the method omits. The calibration
  companion linked above revises only PAIN's normalization path to retain the
  pre-cap aggregate and normalize it by the High-requirement maximum; it does not
  alter the published CVSS score or vector.
...
\item \textbf{Scope is purely hierarchical.} An asset is multi-agency only if the
  ...
  multi-agency --- is rejected in favor of explicit, operator-controlled tagging.)\par\smallskip
\item \textbf{``Archetype'', not ``type'' --- and archetype is an
  implementation choice.} The formula ultimately requires only a per-asset
  Security Requirements vector (\code{CR/IR/AR}); it does not require
  \code{asset-archetype}, or any particular metadata key. Any governed metadata
  label or classification scheme can supply that vector if each supported value
  maps deterministically to a documented security-requirements gradient. We
  recommend \code{asset-archetype} because classifying assets by their \emph{role/usage
  pattern}, rather than asking an owner to state their value directly, provides
  more granularity and a clearer, auditable reasoning chain. The same software can
  therefore land in different archetypes by how it is used (an in-memory store is \code{app-tier}
  as a cache, \code{identity-secrets} as a session store, or
  ...
  estate. An adopter \rfckw{may} instead use another metadata vocabulary, provided
  its mappings and defaults are governed and auditable; if it retains an archetype
  taxonomy, the role-not-kind semantics should be preserved.
...
1.2 & July 9, 2026 & M. Venne & Added the optional \code{asset-value} assignment
  scheme as a simpler alternative to \code{asset-archetype}. \\
1.3 & July 10, 2026 & M. Venne & Expanded the mathematical walkthrough of
  Eqs.~\eqref{eq:isc}--\eqref{eq:s} and delineated departures from standard CVSS
  v3.1 scoring. \\
1.4 & July 13, 2026 & M. Venne & Changed the recommended EPSS
  Likely-Exploitable threshold from $0.70$ to $0.50$ and updated the worked
  remediation examples. Clarified that the scoring formula requires a per-asset
  Security Requirements vector, while \code{asset-archetype} is an optional,
  granular metadata mapping chosen for its auditable rationale. \\
1.5 & July 15, 2026 & M. Venne & Refined the direct-IRV
  unauthenticated-automation floor to \code{AV:N/AC:L/PR:N/UI:N}, while retaining
  EPSS at or above $0.50$ and CISA KEV membership as independent LEV predicates. \\
1.6 & July 15, 2026 & M. Venne & Split the edge archetype: \code{public-edge}
  now names the TLS-terminating L7 edge (CR:M/IR:M/AR:H); added
  \code{passthrough-edge} (CR:L/IR:L/AR:H) for edges that never terminate.
  Expanded the catalog commentary with the reasoning. \\
1.7 & July 20, 2026 & M. Venne & Recommended role-based archetypes over direct
  asset-value labels because they expose the factual and governed basis for
  CR/IR/AR while retaining asset value as a conforming fallback. \\
1.8 & July 21, 2026 & M. Venne & Linked the evidence-backed PAIN calibration
  companion and recorded its revised High-centered normalization and derived
  word-threshold parameter set. \\
\bottomrule
\end{tabular}
