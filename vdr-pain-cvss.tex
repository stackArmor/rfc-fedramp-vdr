...
A simpler, equally valid implementation is to tag assets directly with an
\code{asset-value} instead of an \code{asset-archetype}. Under that approach,
...
audit, and it may be sufficient for smaller or more homogeneous estates. Its tradeoff
is that it loses some granularity and the explanatory chain of thought that an
archetype catalog provides: every security requirement moves together rather than
separately modeling confidentiality, integrity, and availability needs.
...
\item \textbf{``Archetype'', not ``type'' --- and archetype is an
  ...
  maps deterministically to a documented security-requirements gradient. We chose
  \code{asset-archetype} because classifying assets by their \emph{role/usage
  pattern}, rather than their intrinsic kind, provides more granularity and a
  clearer, auditable reasoning chain. The same software can therefore land in
  different archetypes by how it is used (an in-memory store is \code{app-tier}
