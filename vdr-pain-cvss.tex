...
The PAIN derivation of Sections~\ref{sec:math} and~\ref{sec:remediation} is
deterministic and \emph{a priori}: given an asset's CR/IR/AR and the two VER axes, the
inputs are fetched and the N-level and deadline follow by formula. Whether a finding is
\emph{truly} exploitable in the running environment --- as opposed to a false positive,
an unreachable code path, a missing prerequisite configuration, or a surface already
covered by a compensating control --- is a different kind of question. It is an
\emph{a posteriori} determination: the relevant factors cannot be enumerated before the
specific vulnerability is known, so the analysis necessarily follows detection (acutely
so for novel vulnerabilities). This is a claim about \emph{timing}, not about
automation --- the determination may be fully automated or AI/agentic, but it cannot be
precomputed.
