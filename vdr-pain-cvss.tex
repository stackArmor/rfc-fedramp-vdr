...
\]
where $E(a)$ is the \emph{exposed surface} --- the delivery shapes (attack-vector
class, protocol, version) an internet actor can actually get to component $a$,
computed by pushing the internet's traffic through every routing, filtering,
authentication, and \emph{sanitization} hop on the path --- and $X(v)$ is the
\emph{required surface} the vulnerability needs in order to fire. Every factor
defaults to reachable absent deterministic evidence. Sanitization verified in
assessment (code review, SAST, unit tests, and the recurring DAST scanning
continuous monitoring already requires) removes the neutralized vulnerability
classes from downstream surfaces. A separate threat-intelligence promotion rule
applies when a CVE is later determined to be indirectly triggerable by
public-internet input; that rule promotes affected findings to IRV without adding a
new term to this equation. Two corrections to common intuitions:
...
alike) excludes a backend from IRV only in its \emph{remote-access} role, never as
customer authentication for the service itself --- the latter is an exploitability
signal, not an IRV exclusion. The full model, evidence requirements, and worked
cases are specified in the companion paper \emph{A Finding-Level Model for
...
companion model's data-flow evidence) flips IRV to NIRV; a
...
\item[{[EGR]}] M.~Venne, \emph{A Finding-Level Model for Internet Reachability
  under the FedRAMP VDR/VER Rules}, stackArmor,
