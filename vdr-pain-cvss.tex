...
mechanism (payload-carried vs.\ peer-bound), and \emph{every term defaults to
...
\item \textbf{CISA Vulnrichment is deliberately excluded.} Earlier
  drafts consumed two CISA Vulnrichment SSVC values: the exploitation state (as a
  LEV input) and the technical impact (as an impact floor). Both were removed. The
  exploitation role is now served by KEV membership --- the instrument BOD~26-04
  itself names --- and the technical-impact floor was dropped because an opaque,
