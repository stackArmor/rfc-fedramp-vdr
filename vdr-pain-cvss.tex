...
Provider (CSP) \rfckw{should} derive its own archetype mapping from the role of each
asset inside the offering. An intended-use Security Requirements ceiling, derived
independently from the offering's and deploying agency's confirmed federal
information types, caps that archetype vector per objective before scoring. This
keeps architectural roles reusable across agency deployments while preserving the
agency-specific C/I/A consequence. A reference architecture and a sample scan
complete the memo.
\end{abstract}
...
\item[Archetype requirements] The uncapped CR/IR/AR vector derived from an asset's
  role and consequence inside the Cloud Service Offering.
...
\item[Effective requirements] The per-objective minimum of the archetype
  requirements and the ceiling; these values enter the PAIN equation.
...
Requirements ceiling over the asset archetype \\
...
risk algebra. It need only (a) assign each asset an archetype CR/IR/AR vector,
...
effective CR/IR/AR. \textbf{SCOPE} answers ``how many agencies' data does the asset hold?''
...
inputs come together. Set the intended-use ceiling, choose an asset archetype,
observe the resulting effective CR/IR/AR vector, and then follow vulnerability
...
An asset archetype and an agency's intended information use answer different
questions. The archetype describes what compromise of the component can do inside
the CSO architecture. The intended-use ceiling describes the greatest federal
consequence supported by the information types that the offering is designed and
authorized to process and that the affected agency intends and is authorized to
place there.
...
one vector per affected agency scope. Let $R_x(o)$ be asset $x$'s archetype
requirement for objective $o\in\{C,I,A\}$ and let $E_x(o)$ be the resolved
...
\begin{plain}
The role says how important the component can be inside the service. The ceiling
says what federal consequences are actually in scope for the affected customer or
customers. PAIN uses the lower value on each of C, I, and A. A High ceiling never
makes a Low archetype High, and a High archetype never manufactures High agency
impact when the intended use is Moderate.
\end{plain}
...
archetype vector as a conservative compatibility mode, because a ceiling can only
...
archetype vector, ceiling and source when present, effective vector, and whether
any objective was capped.
...
\item $\mathrm{CR},\mathrm{IR},\mathrm{AR}$ --- how much \emph{this system} needs
  ...
  caps the governed archetype or asset-value vector by the intended-use ceiling.
...
Likewise, an absent intended-use ceiling leaves the resolved archetype uncapped;
...
\end{plain}
We use $C,I,A$ for impacts. Each example separately states the uncapped archetype
vector $R_x$, the intended-use ceiling $E_x$, and the effective CR/IR/AR vector
$R_x^{\mathrm{eff}}=\min(R_x,E_x)$ that enters PAIN. All arithmetic follows
...
Keep the vulnerability, H/H/H datastore archetype, and multi-agency scope from
...
finding. The high-role archetype remains reusable; the effective requirements
reflect what the confirmed intended use puts at stake.

\paragraph{Example 3 --- the same CVE on a sandbox, single-agency.}
$C\!=\!I\!=\!A\!=\!0.56$, $R_x=(L,L,L)$, and $E_x=(M,M,M)$.
The ceiling is above the archetype and therefore does not raise it:
...
The governed archetype mapping then turns those facts into an \emph{uncapped}
CR/IR/AR vector. A reviewer can check both parts of the architectural decision:
whether the asset performs the stated role and whether that role maps to the right
requirements inside the CSO. Equation~\eqref{eq:effective-requirement} then applies
the independently derived intended-use ceiling. Archetypes do not remove human
judgment, but they make the role judgment visible and harder to manipulate.
Tagging an asset with its archetype also avoids evaluating every asset in isolation
and keeps the architectural rationale readily understandable and auditable. Not
every asset will fit cleanly, so CSPs \rfckw{may} define custom archetypes or adjust
the scoring of existing ones to map their estate accurately.
...
implementation \rfckw{must} derive its archetype assignments from the roles,
trust relationships, control functions, and consequences inside its own CSO
architecture. It \rfckw{may} adopt the example catalog as a starting point, but
each CSP \rfckw{should} own and justify its mapping. The agency-specific FIPS~199
and information-type context belongs in the separately governed Security
Requirements ceiling. Two providers can legitimately assign the same software
different archetypes because it performs different roles; two agency deployments
can retain the same archetype while resolving different effective requirements.
\end{quote}
...
\item \textbf{``Archetype'', not ``type'' --- and archetype is an
  ...
  effective Security Requirements vector (\code{CR/IR/AR}); it does not require
  \code{asset-archetype}, or any particular metadata key. Any governed metadata
  label or classification scheme can supply the uncapped asset vector if each
  supported value maps deterministically to a documented security-requirements
  gradient. We
...
\item \textbf{The intended-use ceiling is separate from the archetype.}
  Equation~\eqref{eq:effective-requirement} caps the asset's role-based vector by
  the confirmed, dimensional federal consequence of the affected agency scope.
  This lets one CSO reuse an archetype catalog across agency deployments without
...
\item \textbf{The archetype catalog and its CR/IR/AR values
  ...
  its own from component roles and consequences inside its CSO architecture, then
  apply the separately governed intended-use ceiling. A 3PAO \rfckw{may} take on the role of
...
1.3 & July 10, 2026 & M. Venne & Expanded the mathematical walkthrough of
  Eqs.~\eqref{eq:u}--\eqref{eq:s} and delineated departures from standard CVSS
  v3.1 scoring. \\
...
1.8 & July 21, 2026 & M. Venne & Made the evidence-backed, High-centered PAIN
  normalization and derived word thresholds operative throughout the method,
  worked examples, and sample scans; linked the calibration companion and retained
  the initial values as historical context. \\
1.9 & July 27, 2026 & M. Venne & Added the intended-use Security Requirements
  ceiling as the explicit upstream input to PAIN. Separated reusable,
  system-relative archetype requirements from agency-specific federal information
  types; effective CR/IR/AR is now their per-objective minimum. \\
2.0 & July 27, 2026 & M. Venne & Defined Certification Class as the provider's
  operational assurance commitment rather than an impact ceiling. Clarified that
  compound dimensional impact is a governed proxy for vulnerability breadth,
  excluded CWE ID as a PAIN severity input, and documented how CSPs may govern and
  disclose alternate threshold calibrations. \\
2.1 & July 27, 2026 & M. Venne & Clarified why genuinely FIPS~199 Low or Moderate
  systems cannot reach Debilitating under the reference calibration while
  Class~C High-objective uses can. Added the Availability-specific design,
  recovery, and contract assurance case for AR:H use below Class~D. \\
\bottomrule
\end{tabular}
