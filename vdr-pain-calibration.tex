...
The governed asset-archetype catalog is also an external input to this calibration.
This paper does not define or revise archetype CR/IR/AR assignments; it holds the
adopted catalog constant while comparing scoring methods.
...
to boundary and ordinary cases. Requirement vectors are explicit scenario inputs
supplied by the governed asset catalog. Results assume a single-agency resource;
...
model simple: one role-based archetype tag supplies CR/IR/AR; no additional scoring
profile is required.
...
\item A scalar cannot replace architecture review. Archetype assignment, Modified
  impact evidence, agency scope, and vulnerability applicability remain governed
  inputs.
